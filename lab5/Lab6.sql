SELECT DISTINCT s.snum, sname FROM salespeople s , customers couter WHERE EXIST (SELECT * FROM customers cinner WHERE cinner.snum = couter.snum AND cinner.cnum <> couter.cnum) AND s.snum=couter.snum;
...
SELECT pno FROM works_on, employee WHERE lname = 'Smith' AND works_on.essn = employee.ssn UNION SELECT pnumber FROM department, project, employee
